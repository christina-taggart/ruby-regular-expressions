...
def grab_ssn(string)
  puts string.match(/\d{3}-?\d{2}-?\d{4}/).to_s
end
...
def hide_all_ssns(string)
  ssn_array = string.scan(/\d{3}-?\d{2}-?\d{4}/).to_a
  ssn_array.each do |ssn|
